...
class MultiScaleCifLossCriterionConfig(FairseqDataclass):
    ...
    align_weight: float = field(
        default = 1.0,
        metadata = {"help": "weight of ctc alignment loss in phone cif"}
    )
...
class LexiconConstraintMultiLevelCifLossCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        align_loss = torch.tensor(0.0).type_as(p_target)
        ctc_align_out = phone_cif_outputs["ctc_align_out"]
        if self.apply_align_loss and ctc_align_out is not None:
            align_loss = self.computeAlignloss(ctc_align_out)
        ...
        phone_loss = p_ce_loss + self.qua_weight * p_qua_loss + self.ctc_weight * ctc_loss + self.align_weight * align_loss
    ...
    def computeAlignloss(self, ctc_align_out):
        align_self_padding_mask = ~(ctc_align_out.eq(0.0))
        align_loss = torch.abs(ctc_align_out - 1.0) * align_self_padding_mask
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "align_loss", align_loss_sum / p_sample_size / math.log(2), p_sample_size, round=3
        )
